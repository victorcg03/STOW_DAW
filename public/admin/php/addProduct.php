...
if ($SESION_INICIADA && $_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    if (!isset($_FILES['imagenes']) || empty($_FILES['imagenes']['name'][0])) {
      echo json_encode(["error" => "No se han subido imágenes"]);
      die();
    }
...
function procesarImagenes($imagenes, $id)
{
  ...
  if (!is_writable($ruta)) {
    echo json_encode(["error" => "No se puede escribir en la carpeta de imágenes"]);
    die();
  }

  foreach ($imagenes['name'] as $key => $imagen) {
    $tipoArchivo = strtolower(pathinfo($imagenes['name'][$key], PATHINFO_EXTENSION));
    ...
    $nombreUnico = $id . "_" . uniqid() . "_" . basename($imagen);
